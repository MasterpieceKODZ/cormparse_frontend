...
import IconHamburgerDots from "@/ui_components/icons/hamburger.dots";
			...
			<div className=" tw-w-max tw-h-max tw-relative">
				...
				<div
					...
					<button
						...
						onClick={
							rank == "lead"
								? () => initiateTeamLeadDemotion(index, username)
								: () => initiateTeamMemberPromotion(index, username, position)
						}>
						{rank == "lead" ? "demote" : "promote"}
					</button>
					<button
						className=" tw-text-gray-700 dark:tw-text-gray-100 tw-text-[16px] tw-font-russo-one tw-my-5 tw-block tw-w-full"
						onClick={() => removeUserFromTeam(index, username, rank, position)}>
						remove
					</button>
				</div>
				<div
					...
					<div className=" tw-w-full tw-h-max tw-mt-4 tw-flex tw-justify-between tw-items-center">
						...
						<button
							...
							onClick={
								rank == "lead"
									? () => hideTeamLeadOptionsPopup(index)
									: () => promoteTeamMemberToLead(index, username, position)
							}>
							proceed
						</button>
					</div>
